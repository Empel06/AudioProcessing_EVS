...
// Singleton
static Rotary_enc rotary_singleton = {
    .counter = 0,
    .debounce = 0,
    .PS = 0,
    .CS = 0
...
Rotary_enc* Rotary_enc_instance() {
    ...
    if (!initialized) {
        ...
        XGpio_SetDataDirection(&rotary_singleton.rotary, 1, 0xFFFFFFFF);  // inputs
...
u32 Rotary_enc_GetValue(Rotary_enc* re) {
    ...
    if (CS == 0b111) {
        switch (re->PS) {
            ...
            case 0b011:
                ...
                xil_printf("RE: rechts, %d\r\n", re->counter);
                state = RE_RIGHT;
                break;

            case 0b101:
                I32decrement(re->counter, 1);
                xil_printf("RE: links, %d\r\n", re->counter);
                state = RE_LEFT;
                break;
...
void Rotary_enc_SetCounter(Rotary_enc* re, u32 val) {
    re->counter = val;
}
...
int main() {
    ...
    // Rotary encoder GPIO init
    XGpio_Initialize(&rotary, XPAR_AXI_GPIO_0_DEVICE_ID);
    XGpio_SetDataDirection(&rotary, 1, 0xFFFFFFFF); // alle pins input

    xil_printf("Start volume: %d%%\r\n", volume);
    ...
    while (1) {
    	...
    	Rotary_State state = Rotary_enc_GetState(re);
